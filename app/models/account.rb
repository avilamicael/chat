...
class Account < ApplicationRecord
  ...
  after_create_commit :notify_creation, :start_trial_if_configured
  ...
  def captain_usage_summary(period: :current_month)
    ...
    scope = scope.where(created_at: Time.current.beginning_of_month..Time.current) if period == :current_month
    {
      ...
      cost_brl: scope.sum(:cost_micros) / 1_000_000.0,
      cost_usd: scope.sum(:cost_usd_micros) / 1_000_000.0
    }
  end
  ...
  def start_trial_if_configured
    days = ENV['TRIAL_PERIOD_DAYS'].to_i
    return unless days.positive?

    Billing::StartTrialService.new(self, days: days).perform
  rescue StandardError => e
    ChatwootExceptionTracker.new(e, account: self).capture_exception
  end

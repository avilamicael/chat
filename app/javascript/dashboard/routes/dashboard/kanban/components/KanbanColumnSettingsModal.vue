  ...
  (actions || []).forEach(action => {
    ...
    switch (action.action_name) {
      case 'auto_create_task':
        form.auto_create_task = true;
        break;
      case 'auto_assign_agent':
        form.auto_assign_agent = true;
        break;
      case 'auto_assign_conversation':
        form.auto_assign_conversation = true;
        break;
      case 'auto_resolve':
...
const buildEnterActions = form => {
  ...
  if (form.auto_assign_agent)
    actions.push({ action_name: 'auto_assign_agent' });
  if (form.auto_assign_conversation)
    actions.push({ action_name: 'auto_assign_conversation' });
  if (form.auto_resolve) actions.push({ action_name: 'auto_resolve' });
  if (form.send_webhook && form.webhook_url)
    actions.push({ action_name: 'send_webhook', url: form.webhook_url });
  if (form.assign_agent && form.agent_id)
    actions.push({ action_name: 'assign_agent', agent_id: form.agent_id });
  if (form.assign_team && form.team_id)
    actions.push({ action_name: 'assign_team', team_id: form.team_id });
  return actions;
};
...
<template>
  ...
  >
    <div
      class="w-full max-w-lg bg-n-solid-1 rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[85vh]"
    >
      <!-- Header -->
      <div
      ...
      <div class="flex-1 overflow-y-auto px-6 py-4 space-y-3">
        ...
        <!-- WIP Limit -->
        <p
          class="text-xs font-semibold text-n-slate-10 uppercase tracking-wide pt-2"
        >
        ...
        >
          <template v-if="enterForm.send_webhook" #editor>
            ...
            <p class="mt-2 text-xs text-n-slate-9">
              {{ t('KANBAN.COLUMN.WEBHOOK_FUTURE_NOTE') }}
            </p>
          </template>
        </SettingsToggleSection>
      ...
      <!-- Footer -->
      <div
        class="flex justify-end gap-2 px-6 py-4 border-t border-n-weak flex-shrink-0"
      >
        <button

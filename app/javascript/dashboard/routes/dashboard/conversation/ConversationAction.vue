...
import ConversationKanbanAction from './ConversationKanbanAction.vue';
...
export default {
  ...
  computed: {
    ...
    accountId() {
      return (
        this.$store.getters['auth/getCurrentAccount']?.id ||
...
<template>
  <div>
    ...
    <ContactDetailsItem
      compact
      :title="$t('KANBAN.CONVERSATION_ACTION.TITLE')"
    />
    <ConversationKanbanAction :conversation-id="conversationId" />
  </div>
</template>
